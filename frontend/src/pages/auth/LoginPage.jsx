...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(
        `Welcome back, ${res.user?.store_name || res.user?.name || res.user?.email}! Signed in successfully.`
      );
    } catch (err) {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen lg:h-screen w-full bg-white sm:bg-[#f8f8fb] lg:bg-[#060c1c] overflow-y-auto lg:overflow-hidden">
      {/* Left Column: Figma 3D Hero Banner (Hidden on Mobile < lg) */}
      <HeroBanner />

      {/* Right Column: Responsive Mobile / Desktop Form Container (Node 404:282 & Node 100:193) */}
      <div className="flex-1 lg:flex-[0.95] flex flex-col items-center justify-between min-h-screen lg:min-h-full bg-white sm:bg-[#f8f8fb] px-4 py-6 sm:p-6 lg:p-8 overflow-y-auto no-scrollbar">
        {/* Main Card */}
        <div className="w-full max-w-[360px] sm:max-w-[450px] bg-white sm:rounded-[16px] sm:p-8 sm:shadow-[0px_4px_20px_rgba(15,34,76,0.04)] sm:border sm:border-[#eaebf0]/60 my-auto flex flex-col justify-between" data-node-id="404:282">
          <div>
            {/* Mobile Header Brand Logo (Visible on mobile screens < lg, Node 404:284) */}
